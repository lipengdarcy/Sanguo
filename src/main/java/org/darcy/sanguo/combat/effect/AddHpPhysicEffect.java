...
import org.darcy.sanguo.unit.Unit;
 ...
 public class AddHpPhysicEffect extends Effect
 {
   private int rate;
   private int minCureRate;
   private int maxCureRate;
 
   public AddHpPhysicEffect(int id, int type, String description, int paramCount, int catagory)
   {
   ...
   public Effect copy()
   {
     AddHpPhysicEffect e = new AddHpPhysicEffect(this.id, this.type, this.description, this.paramCount, this.catagory);
   ...
   public int[] effectBuff(Unit owner, Unit caster, Section section)
   {
     ...
     if (ran < this.rate) {
       ...
       int cure = (caster.getAttributes().get(6) + caster.getAttributes().get(4)) * value / 100 + 
