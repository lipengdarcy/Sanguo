...
import org.darcy.sanguo.util.RandomBox;
 ...
 public class PhyDamageBuffEffect extends Effect
 {
   private int minDamageRate;
   private int maxDamageRate;
 
   public PhyDamageBuffEffect(int id, int type, String description, int paramCount, int catagory)
   {
   ...
   public Effect copy()
   {
     PhyDamageBuffEffect e = new PhyDamageBuffEffect(this.id, this.type, this.description, this.paramCount, this.catagory);
   ...
   private int damageCalc(Unit src, Unit target, int damageRate, Section section) {
     ...
     int basePhyDamage = 0;
     basePhyDamage = (src.getAttributes().get(6) + src.getAttributes().get(4)) * damageRate / 100 - 
       target.getAttributes().get(8);
     Platform.getLog().logCombat(src.getName() + " CombatContent: atk " + (src.getAttributes().get(6) + src.getAttributes().get(4)));
     Platform.getLog().logCombat(target.getName() + " CombatContent: def " + target.getAttributes().get(8));
 
     Platform.getLog().logCombat("CombatContent: rate " + damageRate);
     Platform.getLog().logCombat("CombatContent: phy damage " + basePhyDamage);
 
     int baseDamage = basePhyDamage;
     if (baseDamage < 0) {
